...
print("[INFO] Loading the face recognizer...")
...
# Initialize the video stream, then allow the camera sensor warm up
...
while True:
    ...
    frame = vs.read()
    # Resize the frame to have a width of 600 pixels (while maintaining the aspect ratio), and then
    ...
    # Apply the OpenCV's deep learning based face detector to localize the faces in the input image
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > args["confidence"]:
            ...
            if fW < 20 or fH < 20:
                continue
            # Construct a blob for the face ROI, then pass the blob through the face embedding model to obtain 128-d
            # quantification of the face
            faceBlob = cv2.dnn.blobFromImage(cv2.resize(face,(96, 96)), 1.0 / 255, (96, 96), (0, 0, 0),
                                             swapRB=True, crop=False)
            embedder.setInput(faceBlob)
            vec = embedder.forward()
            # Perform classification to recognize the face
            preds = recognizer.predict_probs(vec)[0]
            j = np.argmax(preds)
            proba = preds[j]
            name = le.classes_[j]
    ...
    # If the 'q' key was pressed, break out from the loop
    if key == ord("q"):
        break
