...
for (i, imgPath) in enumerate(imagePaths):
    ...
    if len(detections) > 0:
        # We are making the assumption that each image has only ONE face, so we will find the bounding box with
        ...
        if confidence > args['confidence']:
            ...
            if fW < 20 or fH < 20:
                continue
            # Construct a blob for the face ROI, then pass the blob through the face embedding model to obtain the 128-d
            # quantification of the face
            faceBlob = cv2.dnn.blobFromImage(face, 1.0 / 255, (96, 96), swapRB=True, crop=False)
